...
public class TabSkillIQLeader extends Fragment {

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        final Context context = getContext();
        View rootView = inflater.inflate(R.layout.tab_skill_iq_leader, container, false);

        ProgressBar progressBar = rootView.findViewById(R.id.progressBar);

        final RecyclerView recyclerView = rootView.findViewById(R.id.skillIQRecyclerView);
        recyclerView.setLayoutManager(new LinearLayoutManager(getActivity()));

        LeaderBoardService leaderBoardService = ServiceBuilder.buildService(LeaderBoardService.class);
        Call<List<Learner>> request = leaderBoardService.getSkillIQLeaders();
        ...
        request.enqueue(new Callback<List<Learner>>() {
            ...
            public void onResponse(Call<List<Learner>> request, Response<List<Learner>> response) {
                ...
                if (response.isSuccessful()) {
                    recyclerView.setAdapter(new LeadersRecyclerAdapter(response.body(), context));
